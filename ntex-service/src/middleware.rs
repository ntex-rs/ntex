...
/// Apply middleware to a service that doens't transform the request type.
pub fn apply<T, S, Req, C, U>(t: T, factory: U) -> ApplyMiddleware<T, S, Req, C>
where
    S: ServiceFactory<Req, C>,
    T: Middleware<S::Service>,
    T::Service: Service<Req>,
