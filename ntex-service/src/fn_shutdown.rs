use std::{cell::Cell, fmt, future::Future, future::ready, marker::PhantomData};

use crate::{Service, ServiceCtx, ServiceFactory};

#[inline]
/// Create `FnShutdown` for function that can act as a `on_shutdown` callback.
pub fn fn_shutdown<St, Req, Err, F>(f: F) -> FnShutdown<St, Req, Err, F>
where
    F: AsyncFnOnce(),
{
    FnShutdown::new(f)
}

pub struct FnShutdown<St, Req, Err, F> {
    f_shutdown: Cell<Option<F>>,
    _t: PhantomData<(St, Req, Err)>,
}

impl<St, Req, Err, F> FnShutdown<St, Req, Err, F> {
    pub(crate) fn new(f: F) -> Self {
        Self {
            f_shutdown: Cell::new(Some(f)),
            _t: PhantomData,
        }
    }
}

impl<St, Req, Err, F> Clone for FnShutdown<St, Req, Err, F>
where
    F: Clone,
{
    #[inline]
    fn clone(&self) -> Self {
        let f = self.f_shutdown.take();
        self.f_shutdown.set(f.clone());
        Self {
            f_shutdown: Cell::new(f),
            _t: PhantomData,
        }
    }
}

impl<St, Req, Err, F> fmt::Debug for FnShutdown<St, Req, Err, F> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("FnShutdown")
            .field("fn", &std::any::type_name::<F>())
            .finish()
    }
}

impl<St, Req, Err, C, F> ServiceFactory<St, Req, C> for FnShutdown<St, Req, Err, F>
where
    F: AsyncFnOnce() + Clone,
{
    type Res = Req;
    type Error = Err;
    type Service = FnShutdown<St, Req, Err, F>;
    type InitError = ();

    #[inline]
    fn create(&self, _: C) -> impl Future<Output = Result<Self::Service, Self::InitError>> {
        if let Some(f) = self.f_shutdown.take() {
            self.f_shutdown.set(Some(f.clone()));
            ready(Ok(FnShutdown {
                f_shutdown: Cell::new(Some(f)),
                _t: PhantomData,
            }))
        } else {
            panic!("FnShutdown was used already");
        }
    }
}

impl<St, Req, Err, F> Service for FnShutdown<St, Req, Err, F>
where
    F: AsyncFnOnce(),
{
    type St = St;
    type Req = Req;
    type Res = Req;
    type Error = Err;

    #[inline]
    async fn shutdown(&self) {
        if let Some(f) = self.f_shutdown.take() {
            (f)().await;
        }
    }

    #[inline]
    fn call(
        &self,
        req: Req,
        _: ServiceCtx<'_, Self>,
    ) -> impl Future<Output = Result<Req, Err>> {
        ready(Ok(req))
    }
}

#[cfg(test)]
mod tests {
    use std::{future::poll_fn, rc::Rc};

    use crate::{chain_factory, fn_service};

    use super::*;

    #[ntex::test]
    async fn test_fn_shutdown() {
        let is_called = Rc::new(Cell::new(false));
        let srv = fn_service(|()| async { Ok::<_, ()>("pipe") });
        let is_called2 = is_called.clone();
        let on_shutdown = fn_shutdown(async move || {
            is_called2.set(true);
        });

        let pipe = chain_factory(srv)
            .and_then(on_shutdown)
            .clone()
            .pipeline(())
            .await
            .unwrap();

        let res = pipe.call((), &()).await;
        assert_eq!(pipe.ready(&()).await, Ok(()));
        assert!(res.is_ok());
        assert_eq!(res.unwrap(), "pipe");
        assert!(!pipe.is_shutdown());
        pipe.shutdown().await;
        assert!(is_called.get());
        assert!(!pipe.is_shutdown());

        let pipe = pipe.bind(());
        poll_fn(|cx| pipe.poll_shutdown(cx)).await;
        assert!(pipe.is_shutdown());

        let _ = format!("{pipe:?}");
    }

    #[ntex::test]
    #[should_panic]
    #[allow(clippy::should_panic_without_expect)]
    async fn test_fn_shutdown_panic() {
        let is_called = Rc::new(Cell::new(false));
        let is_called2 = is_called.clone();
        let on_shutdown = fn_shutdown::<(), (), _>(async move || {
            is_called2.set(true);
        });

        let pipe = chain_factory(on_shutdown).pipeline(()).await.unwrap();
        pipe.shutdown().await;
        assert!(is_called.get());
        assert!(!pipe.is_shutdown());

        let _factory = pipe.get_ref().create(()).await;
    }
}
