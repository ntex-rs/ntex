...
pub(crate) async fn shutdown<A, AR, B, BR>(
    svc1: &A,
    svc2: &B,
    data1: &A::Data,
    data2: &B::Data,
    ...
    B: Service<BR>,
{
    let mut fut1 = pin::pin!(svc1.shutdown(data1));
    let mut fut2 = pin::pin!(svc2.shutdown(data2));
...
{
    let mut fut1 = pin::pin!(ctx.ready(svc1, data1));
    let mut fut2 = pin::pin!(ctx.ready(svc2, data2));
