...
use crate::{Service, ServiceCtx, ServiceState};
    ...
    B: Service<BR, Error = A::Error>,
{
    match st {
        ...
        ServiceState::NotReady => {
            let mut fut1 = pin::pin!(ctx.state(svc1, ServiceState::NotReady));
            let mut fut2 = pin::pin!(ctx.state(svc2, ServiceState::NotReady));

            poll_fn(move |cx| {
                if pin::Pin::new(&mut fut1).poll(cx)?.is_ready() {
                    Poll::Ready(Ok(()))
                } else if pin::Pin::new(&mut fut2).poll(cx)?.is_ready() {
                    Poll::Ready(Ok(()))
                } else {
                    Poll::Pending
                }
