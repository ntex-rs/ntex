...
pub trait WebResponseError<T>: Sized + Error + 'static {
...
mod tests {
    ...
    fn test_into_error() {
        ...
        let s = format!("{e}");
        assert!(s.contains("web::DefaultError"));
    ...
    fn test_other_errors() {
        ...
        assert_eq!(
            WebResponseError::<DefaultError>::error_response(err, &req).status(),
    ...
    fn test_io_error() {
        ...
        assert_eq!(
            ...
            WebResponseError::error_response(io::Error::new(io::ErrorKind::NotFound, ""), &req)
                .status(),
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            WebResponseError::error_response(io::Error::other(""), &req).status(),
        );
    }
    ...
    fn test_urlencoded_error() {
        let req = TestRequest::default().to_http_request();
        let resp: HttpResponse =
            WebResponseError::error_response(UrlencodedError::Overflow { size: 0, limit: 0 }, &req);
    ...
    fn test_json_payload_error() {
        let req = TestRequest::default().to_http_request();
        let resp: HttpResponse = WebResponseError::error_response(JsonPayloadError::Overflow, &req);
        assert_eq!(resp.status(), StatusCode::PAYLOAD_TOO_LARGE);
        let resp: HttpResponse =
