...
pub trait WebResponseError: fmt::Display + fmt::Debug + 'static {
...
impl WebResponseError for std::convert::Infallible {}

impl<A, B> WebResponseError for Either<A, B>
where
    A: WebResponseError,
    B: WebResponseError,
{
...
impl<T> WebResponseError for InternalError<T>
where
...
mod tests {
    ...
    fn test_other_errors() {
        ...
        let resp = WebError::from(TimeoutError::<UrlencodedError>::Timeout).error_response(&req);
        assert_eq!(resp.status(), StatusCode::GATEWAY_TIMEOUT);

        let resp = WebError::from(TimeoutError::<UrlencodedError>::Service(
        ...
        let resp =
            WebResponseError::error_response(&ClientError::Connect(ConnectError::Timeout), &req);
        assert_eq!(resp.status(), StatusCode::GATEWAY_TIMEOUT);
        ...
        let resp = WebResponseError::error_response(&ClientError::TunnelNotSupported, &req);
        assert_eq!(resp.status(), StatusCode::INTERNAL_SERVER_ERROR);

        #[cfg(feature = "cookie")]
        {
            let resp: HttpResponse =
                WebResponseError::error_response(&coo_kie::ParseError::EmptyName, &req);
            assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
        }
        ...
        let resp = WebResponseError::error_response(&err, &req);
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
    ...
    fn test_either_error() {
        ...
        let code = WebResponseError::status_code(&err);
    ...
    fn test_io_error() {
        assert_eq!(
            ...
            WebResponseError::status_code(&io::Error::new(io::ErrorKind::NotFound, "")),
        );
        assert_eq!(
            StatusCode::FORBIDDEN,
            WebResponseError::status_code(&io::Error::new(io::ErrorKind::PermissionDenied, "")),
        );
        assert_eq!(
            StatusCode::INTERNAL_SERVER_ERROR,
            WebResponseError::status_code(&io::Error::other("")),
        );
    }
    ...
    fn test_urlencoded_error() {
        ...
        let resp: HttpResponse =
            WebResponseError::error_response(&UrlencodedError::UnknownLength, &req);
        assert_eq!(resp.status(), StatusCode::LENGTH_REQUIRED);
        let resp: HttpResponse =
            WebResponseError::error_response(&UrlencodedError::ContentType, &req);
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
    }
    ...
    fn test_json_payload_error() {
        let req = TestRequest::default().to_http_request();
        let resp: HttpResponse =
            WebResponseError::error_response(&JsonPayloadError::Overflow, &req);
        assert_eq!(resp.status(), StatusCode::PAYLOAD_TOO_LARGE);
        let resp: HttpResponse =
            WebResponseError::error_response(&JsonPayloadError::ContentType, &req);
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
    }
    ...
    fn test_query_payload_error() {
        ...
        let resp: HttpResponse = WebResponseError::error_response(&err, &req);
        assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
        assert_eq!(WebResponseError::status_code(&err), StatusCode::BAD_REQUEST);
    }
    ...
    fn test_path_error() {
        ...
        assert_eq!(WebResponseError::status_code(&err), StatusCode::NOT_FOUND);
    }
