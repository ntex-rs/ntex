...
type BoxedRouteService<Err: ErrorRenderer> = Box<
    dyn Service<
        Request = WebRequest<Err>,
        Response = WebResponse,
        Error = Err::Container,
        Future = LocalBoxFuture<'static, Result<WebResponse, Err::Container>>,
    >,
>;

type BoxedRouteNewService<Err: ErrorRenderer> = Box<
    dyn ServiceFactory<
        Config = (),
        Request = WebRequest<Err>,
        Response = WebResponse,
...
impl<Err: ErrorRenderer> Route<Err> {
    ...
    pub fn new() -> Route<Err> {
        Route {
            service: Box::new(RouteNewService::<_, Err>::new(
                Extract::<_, _, Err>::new(Handler::<_, _, _, _, Err>::new(|| {
...
pub struct CreateRouteService<Err: ErrorRenderer> {
    #[pin]
    fut: RouteFuture<Err>,
    guards: Rc<Vec<Box<dyn Guard>>>,
}

impl<Err: ErrorRenderer> Future for CreateRouteService<Err> {
...
impl<Err: ErrorRenderer> Route<Err> {
    ...
    {
        self.service = Box::new(RouteNewService::<_, Err>::new(Extract::new(
            Handler::new(handler),
        )));
        self
    }
...
where
    T: ServiceFactory<Request = WebRequest<Err>, Error = (Err::Container, HttpRequest)>,
    Err: ErrorRenderer,
{
    service: T,
    _t: PhantomData<Err>,
            ...
            .map(|result| match result {
                Ok(service) => {
                    let service: BoxedRouteService<Err> =
                        Box::new(RouteServiceWrapper::<_, Err> {
                            service,
                            _t: PhantomData,
                        });
                    Ok(service)
...
struct RouteServiceWrapper<
    T: Service<Error = (Err::Container, HttpRequest)>,
    Err: ErrorRenderer,
> {
    service: T,
    _t: PhantomData<Err>,
