...
pub trait ResponseError: error::Error {
    /// Creates an HTTP response for this error.
    ///
    /// The default response has status `500 Internal Server Error`, a
    /// `text/plain; charset=utf-8` content type, and the error's
    /// [`Display`](fmt::Display) representation as its body. Implementations
    /// should override this method when that text is not suitable for clients.
...
pub enum DecodeError {
    ...
    Header,
    /// The message contains more headers than the configured limit.
...
#[derive(thiserror::Error, Debug)]
/// Errors that can occur while processing a payload.
...
/// Errors that can occur while dispatching HTTP requests.
...
/// Errors that can occur while parsing a `Content-Type` header.
...
/// An error produced by a blocking operation.
#[derive(thiserror::Error, Debug)]
pub enum BlockingError<E: fmt::Debug> {
    /// The blocking operation returned an error.
    #[error("{0:?}")]
    Error(#[source] E),
    /// The thread pool stopped before completing the operation.
