...
impl MessageBody {
        ...
        if let Some(l) = res.headers().get(&CONTENT_LENGTH) {
            if let Ok(s) = l.to_str() {
                if let Ok(l) = s.parse::<usize>() {
                    ...
                    return Self::err(
                        Error::from(PayloadError::UnknownLength)
    ...
    fn err(e: Error<PayloadError>, config: ClientConfig) -> Self {
        MessageBody {
            config,
            fut: None,
            err: Some(e),
...
impl Future for MessageBody {
    type Output = Result<Bytes, Error<PayloadError>>;
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if let Some(len) = this.length.take() {
            ...
            if limit > 0 && len > limit {
                return Poll::Ready(Err(Error::from(PayloadError::Overflow)
            ...
            return match Pin::new(&mut this.stream).poll_next(cx) {
                Poll::Ready(Some(Ok(chunk))) => {
                    if this.limit > 0 && (this.buf.len() + chunk.len()) > this.limit {
                        Poll::Ready(Err(Error::from(PayloadError::Overflow)
                            .set_service(this.config.cfg().service())))
                    } else {
                ...
                Poll::Ready(Some(Err(err))) => Poll::Ready(Err(
                    Error::from(err).set_service(this.config.cfg().service())
                )),
                Poll::Pending => {
                    if this.timeout.poll_elapsed(cx).is_ready() {
                        Poll::Ready(Err(Error::from(PayloadError::Incomplete(Some(
                            std::io::Error::new(
