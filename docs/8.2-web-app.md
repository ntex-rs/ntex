...
- **Application state** is long-lived and available to every request handled
  by the application. The server clones it for each connection, so data that
  must be shared between connections belongs behind a shared handle such as
  `Rc`.
...
[Worker and request state](./6-server-app.md) for worker initialization
...
state must implement it. Every way of serving an application clones the state:
[`ServerAppConfig::State`] and state passed to `build_with()` must be `Clone`,
and [`HttpService`] clones the connection state for each connection. A
non-`Clone` state type can be used to define an application, but the
application cannot be served.
...
[Worker-Local and Process-Wide State](./6-server-app.md#worker-local-and-process-wide-state)
for details.
...
[Builder Order](./8.1-web-app.md#builder-order)). You can finish building it
...
[`ServerAppConfig::State`]: https://docs.rs/ntex/latest/ntex/server/trait.ServerAppConfig.html#associatedtype.State
...
[`WebRequest::st()`]: https://docs.rs/ntex/latest/ntex/web/struct.WebRequest.html#method.st
