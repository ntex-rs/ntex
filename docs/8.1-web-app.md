...
The server calls the application factory once for each worker:
...
so application services are never shared between workers. To learn more about workers,
...
application. See [Web Application Filters and Middleware](./8.3-web-app.md) for more
information.
...
`scope("/api")` handles `/api` and every path below `/api/`, but not `/apix`.
Inside the scope, an empty resource pattern (`""`) matches the bare `/api`
path, while `"/"` matches `/api/`:
