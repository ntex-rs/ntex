...
Both can use application state through [`Ctx`]. For the distinction between
application state and request-local state, see [Web Application State](./8.2-web-app-state.md).
...
struct ResponseHeaderService<S>(S);
    ...
    S: Service<St, WebRequest<ReqSt>, Res = WebResponse>,
{
    ...
    async fn call(
        ...
        let mut res = ctx.call(&self.0, req).await?;
...
readiness and lifecycle events correctly. Middleware that owns other services
should also forward readiness and shutdown as described in
...
application router chooses a scope or resource. See
[Web Application Routing](./8.1-web-app.md) for guard and fallback behavior.
...
- The component needs to participate in service readiness or shutdown.
